  ...
  return (
    <div key={key}>
      {formData[selectField(field[0])].map((x, i) => (
        <div className="flex w-full justify-between px-2 py-2 bg-[#ABD0DA] my-2 rounded-xl">
          <h1>{i + 1}.</h1>
          {Object.values(x).map((data) => (
            <h1>{data}</h1>
          ))}
          <button
            className="bg-red-400 px-1 py-1 text-xl rounded"
            onClick={() => {
      ...
      <button
        className="mt-2 w-full bg-[#E24748] text-white font-bold py-2 px-4 rounded"
        onClick={() => add()}
      >
        Add
      </button>
