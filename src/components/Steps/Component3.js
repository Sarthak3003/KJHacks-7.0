    ...
    <div className="flex items-center justify-center">
      <div className="bg-white w-full py-2 px-3 shadow-lg">
        <div className="flex justify-between">
          ...
          <button
            onClick={() => removeGst()}
            class=" bg-purple-gray-500 hover:bg-purple-gray-600 text-white font-bold py-2 px-4 rounded m-5 "
          >
            <CancelIcon />
          </button>
        </div>
...
export default function FormPropsTextFields() {
  ...
  const getStartups = () => {
    let config = {
      method: 'get',
      url: 'https://bdd0-182-76-21-121.in.ngrok.io/account/startup/',
      headers: {
  ...
  //   let config = {
  //     method: 'post',
  //     url: 'https://bdd0-182-76-21-121.in.ngrok.io/account/gstverify/',
  //     headers: {
  //       'Authorization': 'Token ' + token
  //     },
  //     data: {
  ...
  const submit = async () => {
    if (select === 'GST') {
      ...
      var config = {
        method: 'post',
        url: 'https://bdd0-182-76-21-121.in.ngrok.io/account/panverify/',
        headers: {
          Authorization: 'Token ' + token,
        },
        data: data,
        ...
        <div className="pt-2">
          ...
          <div className={`flex'} gap-1`}>
            {['PAN', 'GST', 'CIN'].map((option) => (
              <label
                class={`flex items-center gap-1'} text-sm text-purplezinc`}
              >
                <input
                  type="radio"
                  ...
                  value={option}
                  class="w-4 h-4 accent-purplegray-600 bg-gray-100 border-gray-300"
                />
                {option}
              </label>
            ))}
          ...
          <input
            className="px-3 py-2"
            placeholder={`Enter ${select} Number`}
            value={input}
