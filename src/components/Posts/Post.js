...
const CommentBody = ({ comment, comments }) => {
  ...
  const getComments = (comment) => {
    var config = {
      method: 'get',
      url:
        'https://bdd0-182-76-21-121.in.ngrok.io/api/comments/' + comment + '/',
      headers: {
  ...
  const postComment = () => {
    ...
    var config = {
      method: 'post',
      url: 'https://bdd0-182-76-21-121.in.ngrok.io/api/comments/',
      headers: {
        Authorization: 'Token ' + token,
      },
      data: data,
  ...
  return (
    <div className="p-2">
      <div className="flex w-full">
        <input
          className="px-3 py-2 grow border"
...
const Post = forwardRef(({ name, description, message, photoUrl }, ref) => {
  ...
  const like = (id) => {
    ...
    var config = {
      method: 'post',
      url: 'https://bdd0-182-76-21-121.in.ngrok.io/api/post-like/',
      headers: {
        Authorization: 'Token ' + token,
      },
      data: data,
  ...
  const dislike = () => {
    var config = {
      method: 'delete',
      url: 'https://bdd0-182-76-21-121.in.ngrok.io/api/post-like/' + likedId,
      headers: {
        Authorization: 'Token ' + token,
      },
      data: data,
  ...
  const getPosts = () => {
    var config = {
      method: 'get',
      url: 'https://bdd0-182-76-21-121.in.ngrok.io/api/posts/',
      headers: {
        Authorization: 'Token ' + token,
      },
    }
  ...
  function InputCommentOption({ Icon, title, color, id }) {
    return (
      <div
        className="inputOption"
        onClick={() => (comments === id ? setComments(null) : setComments(id))}
      >
        <Icon style={{ color: color }} />
        <h4>{title}</h4>
      </div>
  ...
  const Post = ({ post }) => {
    return (
      <div ref={ref} key={post.id} className="post shadow-xl">
        ...
        {comments === post.id && (
          <CommentBody comment={post.id} comments={comments} />
        )}
      </div>
    )
  }
